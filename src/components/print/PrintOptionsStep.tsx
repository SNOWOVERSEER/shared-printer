...
const PrintOptionsStep = ({ fileList, form, onPrevious, onNext, filePages, initialValues, setCheckoutPrice, checkoutPrice }: PrintOptionsStepProps) => {
    ...
    useEffect(() => {
        setTotalPrice(priceCalculator(form.getFieldValue('colorMode'), form.getFieldValue('sides'), form.getFieldValue('paperSize'), filePages, form.getFieldValue('copies')).totalPrice);
    }, []);
    ...
    const handleFormChange = (changedValues: any, allValues: any) => {
        try {
            ...
            messageApi.error(`Invalid parameters: ${error}`);
        }
    };
        ...
        <>
            ...
            <div className="fade-in">
                            ...
                            <Col xs={24} md={12}>
                                <Form.Item name="sides" label="Sides" rules={[{ required: true, message: 'Please select sides' }, {
                                    validator: (rule, value, callback) => {
                                        if (filePages < 2 && value === 'double') {
                                            callback('Double sided printing is not allowed for less than 2 pages');
                                        }
                                        callback();
                                    }
                                }]}>
                ...
                <Row justify="space-between">
                    ...
                    >
                        Next: Continue to Delivery Information
                    </Button>
                </Row>
            </div >
        </>
